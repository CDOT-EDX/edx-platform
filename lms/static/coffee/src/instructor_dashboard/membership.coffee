...
class AutoEnrollmentViaCsv
  ...
  display_response: (data_from_server) ->
    ...
    notofication_model = new NotificationModel()
    ...
    render_response = (title, message, type, student_results) =>
      ...
      if type is 'confirmation'
        ...
        view = new NotificationView(model:notofication_model);
        view.render()
    ...
    if errors.length
      render_response 'Errors', gettext("The following errors were generated:"), 'error', errors
    if warnings.length
      render_response 'Warnings', gettext("The following warnings were generated:"), 'warning', warnings
    if result_from_server_is_success
      render_response 'Success', gettext("All accounts were created successfully."), 'confirmation', []
